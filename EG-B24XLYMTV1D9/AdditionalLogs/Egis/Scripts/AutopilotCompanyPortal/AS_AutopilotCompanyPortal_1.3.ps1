	...
	If(-Not(Test-Path "$env:LOCALAPPDATA\Autopilot_CompanyPortal_Flag.txt")){
		$AppxBundle = "C:\ProgramData\EGIS\Scripts\AutopilotCompanyPortal\Microsoft.CompanyPortal_11.2.360.0_neutral_~_8wekyb3d8bbwe.AppxBundle"
		...
		Write-Log -Path "$($env:LocalAppData)\Logs\" -Message "Installing Company Portal and its dependencies for user $($env:USERNAME)"
		Foreach($Dependency in $Dependencies){
			Write-Log -Path "$($env:LocalAppData)\Logs\" -Message "Installing $Dependency"
			Add-AppxPackage -Path $Dependency -ErrorAction Ignore
		}
	
		$Error.Clear()
		Write-Log -Path "$($env:LocalAppData)\Logs\" -Message "Installing $AppxBundle"
		...
		If ($error.count -eq 0){
			...
			Else{
				Write-Host "Shortcut for Company Portal already exists on the user desktop"
			}
